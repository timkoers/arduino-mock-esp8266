...
#include "WiFiClientSecure_BearSSL.h"
...
void WiFiClientSecure_::_clearAuthenticationSettings() {
  _use_insecure = false;
...
WiFiClientSecure_::WiFiClientSecure_() : WiFiClient() {
  _clear();
...
WiFiClientSecure_::WiFiClientSecure_(const WiFiClientSecure_ &rhs) : WiFiClient(rhs) {
  *this = rhs;
  stack_thunk_add_ref();
}

WiFiClientSecure_::~WiFiClientSecure_() {
  if (_client) {
    _client->unref();
    _client = nullptr;
  }
...
WiFiClientSecure_::WiFiClientSecure_(ClientContext* client,
...
void WiFiClientSecure_::setClientRSACert(const X509List *chain, const PrivateKey *sk) {
  _chain = chain;
  _sk = sk;
}

void WiFiClientSecure_::setClientECCert(const X509List *chain,
...
void WiFiClientSecure_::setBufferSizes(int recv, int xmit) {
...
bool WiFiClientSecure_::stop(unsigned int maxWaitMs) {
...
bool WiFiClientSecure_::flush(unsigned int maxWaitMs) {
  (void) _run_until(BR_SSL_SENDAPP);
  return WiFiClient::flush(maxWaitMs);
}

int WiFiClientSecure_::connect(IPAddress ip, uint16_t port) {
...
int WiFiClientSecure_::connect(const char* name, uint16_t port) {
...
int WiFiClientSecure_::connect(const String& host, uint16_t port) {
  return connect(host.c_str(), port);
}

void WiFiClientSecure_::_freeSSL() {
...
bool WiFiClientSecure_::_clientConnected() {
  return (_client && _client->state() == ESTABLISHED);
}

uint8_t WiFiClientSecure_::connected() {
...
size_t WiFiClientSecure_::_write(const uint8_t *buf, size_t size, bool pmem) {
  size_t sent_bytes = 0;
...
size_t WiFiClientSecure_::write(const uint8_t *buf, size_t size) {
  return _write(buf, size, false);
}

size_t WiFiClientSecure_::write_P(PGM_P buf, size_t size) {
  return _write((const uint8_t *)buf, size, true);
}

// We have to manually read and send individual chunks.
size_t WiFiClientSecure_::write(Stream& stream) {
  size_t totalSent = 0;
  size_t countRead;
  size_t countSent;
...
int WiFiClientSecure_::read(uint8_t *buf, size_t size) {
...
int WiFiClientSecure_::available() {
  if (_recvapp_buf) {
...
size_t WiFiClientSecure_::peekBytes(uint8_t *buffer, size_t length) {
  size_t to_copy = 0;
  if (!ctx_present()) {
...
*/
int WiFiClientSecure_::_run_until(unsigned target, bool blocking) {
  if (!ctx_present()) {
...
bool WiFiClientSecure_::_wait_for_handshake() {
  _handshake_done = false;
...
bool WiFiClientSecure_::setFingerprint(const char *fpStr) {
  int idx = 0;
  uint8_t c, d;
  uint8_t fp[20];
...
bool WiFiClientSecure_::setCiphers(const uint16_t *cipherAry, int cipherCount) {
  _cipher_list = nullptr;
...
bool WiFiClientSecure_::_connectSSL(const char* hostName) {
...
bool WiFiClientSecure_::_installServerX509Validator(const X509List *client_CA_ta) {
  if (client_CA_ta) {
    _ta = client_CA_ta;
    // X509 minimal validator.  Checks dates, cert chain for trusted CA, etc.
    _x509_minimal = std::make_shared<br_x509_minimal_context>();
...
bool WiFiClientSecure_::_connectSSLServerRSA(const X509List *chain,
    const PrivateKey *sk,
...
bool WiFiClientSecure_::_connectSSLServerEC(const X509List *chain,
    unsigned cert_issuer_key_type, const PrivateKey *sk,
    const X509List *client_CA_ta) {
#ifndef BEARSSL_SSL_BASIC
  _freeSSL();
...
int WiFiClientSecure_::getLastSSLError(char *dest, size_t len) {
  int err = 0;
...
bool WiFiClientSecure_::probeMaxFragmentLength(const char* name, uint16_t port, uint16_t len) {
  IPAddress remote_addr;
  if (!WiFi.hostByName(name, remote_addr)) {
    DEBUG_BSSL("probeMaxFragmentLength: Can't resolve host\n");
    return false;
  }
  return WiFiClientSecure_::probeMaxFragmentLength(remote_addr, port, len);
}

bool WiFiClientSecure_::probeMaxFragmentLength(const String& host, uint16_t port, uint16_t len) {
  return WiFiClientSecure_::probeMaxFragmentLength(host.c_str(), port, len);
}
...
bool WiFiClientSecure_::probeMaxFragmentLength(IPAddress ip, uint16_t port, uint16_t len) {
...
bool WiFiClientSecure_::setCACert(const uint8_t* pk, size_t size) {
  _axtls_ta = nullptr;
...
bool WiFiClientSecure_::setCertificate(const uint8_t* pk, size_t size) {
  _axtls_chain = nullptr;
...
bool WiFiClientSecure_::setPrivateKey(const uint8_t* pk, size_t size) {
  _axtls_sk = nullptr;
...
uint8_t *WiFiClientSecure_::_streamLoad(Stream& stream, size_t size) {
...
bool WiFiClientSecure_::loadCACert(Stream& stream, size_t size) {
...
bool WiFiClientSecure_::loadCertificate(Stream& stream, size_t size) {
  uint8_t *dest = _streamLoad(stream, size);
  bool ret = false;
  if (dest) {
#pragma GCC diagnostic push
...
bool WiFiClientSecure_::loadPrivateKey(Stream& stream, size_t size) {
  uint8_t *dest = _streamLoad(stream, size);
  bool ret = false;
  if (dest) {
#pragma GCC diagnostic push
